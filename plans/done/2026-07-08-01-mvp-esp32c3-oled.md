...
- **Date:** 2026‑07‑08
- **Status:** ✅ **done** — shipped as [v0.1.0](https://github.com/lightheaded/bledash-esp32/releases/tag/v0.1.0) (PRs #1–#5). Firmware milestones M1–M5 complete; only **M6 (physical car install)** remains as follow-up.
...
- **MINI ESP32‑C3 Development Board with 0.42" OLED** ([€3.34 on AliExpress](https://www.aliexpress.com/item/1005008570438676.html)).
...
- ✅ **M1 — Hello display** (PR #1). u8g2 on the 72×40 OLED; confirmed I²C on GPIO5/6 by boot-time probe.
- ✅ **M2 — BLE scan / recon** (PR #2). Identified the K25 (service `0xFFF0`), both EcoFlows, and the battery‑% offset (19), verified against HA. Findings in `docs/protocols/`.
- ✅ **M3 — Alpicool driver** (PR #3). Held connection + BIND + QUERY/notify parse; fridge page. Verified ON/+2°C against the fridge's own display. Hardened the connection for weak links (5 s supervision timeout + discovery retry).
- ✅ **M4 — EcoFlow advert parser + combined page** (PR #4). Passive scan → serial match → battery %, validated 99% against HA. Single owner-driven scan loop feeds both devices; combined left/right single page.
- ✅ **M5 — Scheduler + single‑page UI polish** (this PR). 60 s cadence, per-device stale detection, px‑shift burn‑in guard. **Single page kept** — it reads well at a glance on the real panel, so the rotating-page fallback is not needed. Tag v0.1.0.
...
2. ~~Does the River 2 Max keep advertising while its WiFi/cloud link is active?~~ **Resolved at M2: yes** while active. **M5 refinement:** after ~1 h idle the Max stops advertising *and* stops reporting to the cloud together — a device power/idle state, not BLE-specific. In use (powering the fridge in the car) it advertises fine; the display shows `--` when the reading goes stale.
