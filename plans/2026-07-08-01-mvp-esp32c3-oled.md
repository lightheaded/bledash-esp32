...
- **Onboard 72×40 SSD1306 OLED**, I²C. Exact pins TBD at M1: community drivers for this board family typically use `SDA=GPIO5`, `SCL=GPIO6` for the onboard OLED, but the vendor pinout below labels I²C as `SDA=GPIO8`, `SCL=GPIO9` (possibly the header breakout, not the OLED). Try both pairs; document the winner.
...
**Tier 1 (MVP): passive advertisement → battery %.** The River 2 series broadcasts manufacturer data (manufacturer ID `46517` / `0xB5B5`) containing the device serial in ASCII (serial prefix `R611` = River 2 Max) and a raw battery‑percent byte (reported at manufacturer‑data offset ~34–36 on the River 2 per `npike/ha-ecoflow-ble`; **verify the exact offset for the Max in M2** by watching the byte track the real SoC / HA cloud sensor). No connection, no auth. Match on the device serial (from `config.h`), not MAC, until the MAC is confirmed stable.
...
1. Which GPIO pair is the onboard OLED on for *this specific clone*? The vendor pinout (`docs/esp32-c3-0.42-oled.png`) labels I²C on GPIO8/9, community OLED code says GPIO5/6 — M1 tests both on real hardware.
2. Does the River 2 Max keep advertising while its WiFi/cloud link is active? HA reads it via cloud today and the adverts are what MVP depends on — M2 verifies. (Research found no evidence BLE turns off when WiFi is provisioned, but it's unconfirmed for this model; anecdotes exist of BLE going idle until a button press after long inactivity.)
3. Exact battery‑% offset in the R611 manufacturer data (M2).
